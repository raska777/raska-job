...
//         const to = isSubscribed.email;
//         const subject = `🔔 Yangi ish e’loni: ${job.work_type}`;
//         const html = `
//           <div>
//             <h2>Assalomu alaykum ${isSubscribed.name || ""},</h2>
//             <p>Platformada yangi ish e’loni joylandi:</p>
//             <p><b>Ish turi:</b> ${job.work_type}</p>
//             <p><b>Joylashuv:</b> ${job.location}</p>
//             <p><b>Ish vaqti:</b> ${job.work_time}</p>
...
import { User } from "@/lib/types"; // User turini import qilish
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (type === "job-posted" && job) {
      ...
      // Foydalanuvchilarni turga moslashtirish
      const emailPromises = (subscribers as User[]).map((subscriber) => {
        const to = subscriber.email;
        const subject = `🔔 Yangi ish e’loni: ${job.work_type}`;
        const html = `
          <div>
            <h2>Assalomu alaykum ${subscriber.name || ""},</h2>
            <p>Platformada yangi ish e’loni joylandi:</p>
            <p><b>Ish turi:</b> ${job.work_type}</p>
            <p><b>Joylashuv:</b> ${job.location}</p>
            <p><b>Ish vaqti:</b> ${job.work_time}</p>
