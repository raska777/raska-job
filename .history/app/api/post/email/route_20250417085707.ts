...
//     const emailPromises = isSubscribed.map((user) => {
//       const to = user.email;
//       const subject = `🔔 obuna: ${job.work_type}`;
//       const text = `Assalomu alaykum ${user.name || ''},
...
// 1. Foydalanuvchi obuna bo‘lganda yuboriladigan email
export async function POST(req: NextRequest) {
  try {
    const body = await req.json();
    const { type, user, job } = body;

    const client = await clientPromise;
    const db = client.db("raska");

    // 1. Obuna bo‘lgan foydalanuvchiga xush kelibsiz email yuborish
    if (type === "welcome" && user) {
      const subject = "🎉 Obunangiz muvaffaqiyatli faollashtirildi!";
      const html = `
        <div>
          <h2>Assalomu alaykum ${user.name || ""},</h2>
          <p>Siz <b>Raska Job</b> platformasiga muvaffaqiyatli obuna bo‘ldingiz.</p>
          <p>Endilikda siz yangi ish e’lonlaridan email orqali xabardor bo‘lib borasiz.</p>
          <p>Hurmat bilan,<br/>Raska Platformasi</p>
        </div>
      `;
      await sendEmail(user.email, subject, html);
      return NextResponse.json({ message: "Xush kelibsiz email yuborildi ✅" });
    }

    // 2. Yangi ish e’loni joylanganda barcha obunachilarga email yuborish
    if (type === "job-posted" && job) {
      const isSubscribed = await db
        .collection("users")
        .find({ isSubscribed: true }) // Obuna bo‘lgan foydalanuvchilarni olish
        .toArray();

      // Foydalanuvchilarga email yuborish
      const emailPromises = isSubscribed.map((subscriber) => {
        const to = subscriber.email; // subscriber.email
        const subject = `🔔 Yangi ish e’loni: ${job.work_type}`;
        const html = `
          <div>
            <h2>Assalomu alaykum ${subscriber.name || ""},</h2> <!-- subscriber.name -->
            <p>Platformada yangi ish e’loni joylandi:</p>
            <p><b>Ish turi:</b> ${job.work_type}</p>
            <p><b>Joylashuv:</b> ${job.location}</p>
            <p><b>Ish vaqti:</b> ${job.work_time}</p>
      ...
      const results = await Promise.allSettled(emailPromises); // Emaillarni yuborishni amalga oshiradi
      const errors = results.filter((res) => res.status === "rejected");

      if (errors.length > 0) {
        console.error("❌ Ayrim emaillar yuborilmadi:", errors);
        return NextResponse.json({ warning: "Qisman xatolik yuz berdi", sent: results.length - errors.length }, { status: 207 });
      }

      return NextResponse.json({ message: "Barcha obunachilarga email yuborildi ✅" });
    }

    return NextResponse.json({ error: "Noto‘g‘ri email turi" }, { status: 400 });
  } catch (err) {
    console.error("❌ Email route xatoligi:", err);
    return NextResponse.json({ error: "Server xatosi" }, { status: 500 });
  }
}
