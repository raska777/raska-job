...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("📨 Yangi obuna bo'lgan foydalanuvchi:", newSubscriber);

    // Faqat yangi obuna bo'lgan foydalanuvchiga email yuboramiz
    if (newSubscriber.email) {
      const to = newSubscriber.email;
      const subject = `🔔 Raska Jobga obuna bo'lganingiz uchun rahmat!`;
      const text = `Assalomu alaykum ${newSubscriber.name || ''}, Raska Jobga obuna bo'lganingiz uchun minnatdormiz. Sizga yangi ish o'rinlari haqida xabarlar yuborib turamiz.`;
      
      await sendEmail(to, subject, text);
    }

    return NextResponse.json(
      { message: "Obuna tasdiqlandi va email yuborildi" }, 
      { status: 200 }
    );
  } catch (err) {
    console.error("❌ Email yuborishda xato:", err);
