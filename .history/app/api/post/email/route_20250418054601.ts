...
export async function POST(request: NextRequest) {
    try {
      ...
      const existingUser = await db
        .collection("users")
        .findOne({ email: newSubscriber.email });
      ...
      // 2️⃣ Agar yangi obuna bo'lsa:
      // ✅ DB-ga yangi foydalanuvchini qo'shish (yoki isSubscribed: true qilish)
      await db.collection("users").updateOne(
        { email: newSubscriber.email },
        { $set: { ...newSubscriber, isSubscribed: true } },
        { upsert: true } // Agar mavjud bo'lmasa, yangi yaratadi
      );
  
      // 3️⃣ Faqat shu foydalanuvchiga email yuborish
      const to = newSubscriber.email;
      const subject = `✅ Raska Jobga obuna bo'lganingiz uchun rahmat!`;
      const text = `Assalomu alaykum ${newSubscriber.name || ''}, Raska Jobga obuna bo'lganingiz uchun minnatdormiz.`;
  
      await sendEmail(to, subject, text);
  
      return NextResponse.json(
        { message: "Obuna tasdiqlandi. Email yuborildi!" },
        { status: 200 }
      );
    } catch (err) {
