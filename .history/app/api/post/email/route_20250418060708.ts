...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("📨 Yangi obuna so'rovi:", newSubscriber);

    const client = await clientPromise;
    const db = client.db("raska");

    // 1️⃣ Foydalanuvchi allaqachon obuna bo'lganmi?
    const existingUser = await db
      .collection("users")
      .findOne({ email: newSubscriber.email, isSubscribed: true });

    if (existingUser) {
      return NextResponse.json(
        { error: "Ushbu email allaqachon obuna bo'lgan" },
        { status: 400 }
      );
    }

    // 2️⃣ Yangi obunachini DB-ga qo'shish (yoki isSubscribed: true qilish)
    await db.collection("users").updateOne(
      { email: newSubscriber.email },
      { $set: { ...newSubscriber, isSubscribed: true } },
      { upsert: true }
    );

    // 3️⃣ Faqat shu foydalanuvchiga "Obuna tasdiqlandi" emaili yuborish
    const welcomeEmail = {
      to: newSubscriber.email,
      subject: `✅ Raska Jobga obuna bo‘lganingiz uchun rahmat!`,
      text: `Assalomu alaykum ${newSubscriber.name || ''}, Raska Jobga obuna bo‘lganingiz uchun minnatdormiz. Sizga yangi ish o‘rinlari haqida xabarlar yuborib turiladi.`,
    };

    await sendEmail(welcomeEmail.to, welcomeEmail.subject, welcomeEmail.text);

    return NextResponse.json(
      { message: "Obuna tasdiqlandi. Xabar yuborildi!" },
      { status: 200 }
    );
  } catch (err) {
    console.error("❌ Xatolik:", err);
    return NextResponse.json(
      { error: "Server xatosi. Iltimos keyinroq urunib ko'ring." },
      { status: 500 }
    );
  }
}