...
export async function POST(request: NextRequest) {
  try {
    const job = await request.json();
    console.log("📨 Yangi obuna bo'lgan foydalanuvchi:", job);

    const client = await clientPromise;
    const db = client.db("raska");

    // 1. Foydalanuvchini obunachilar ro'yxatiga qo'shamiz
    await db.collection("users").updateOne(
      { email: job.email },
      { $set: { ...job, isSubscribed: true } },
      { upsert: true }
    );

    // 2. Faqat shu yangi obunachiga xabar yuboramiz
    const to = job.email;
    const subject = `🔔 Raska Jobga obuna bo'lganingiz uchun rahmat!`;
    const text = `Assalomu alaykum ${job.name || ''}, Raska Jobga obuna bo'lganingiz uchun minnatdormiz. Yangi ish o'rinlari haqida xabarlar sizga yuboriladi.`;
