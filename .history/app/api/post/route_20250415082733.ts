...
import { NextRequest, NextResponse } from "next/server";
import clientPromise from "@/lib/mongodb";
import { sendEmail } from "@/lib/email"; // Email funksiyasi
...
export async function POST(req: NextRequest) {
  try {
    ...
    for (const subscriberEmail of emailList) {
      const subject = "Yangi ish e’loni qo‘shildi!";
      const text = `Salom!

Siz uchun yangi ish e’loni qo‘shildi:
...
Batafsil ma’lumot olish uchun saytimizga tashrif buyuring.

Hurmat bilan,
RaskaJob jamoasi`;

      await sendEmail(subscriberEmail, subject, text);
    }
