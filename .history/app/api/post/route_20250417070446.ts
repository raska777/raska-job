...
// app/api/post/route.ts - ichida
import sendEmail from "@/lib/email"; // bu importni yuqoriga qo‘sh
...
export async function POST(req: NextRequest) {
  try {
    ...
    // ✅ Email yuborish (faqat obuna bo‘lgan foydalanuvchilarga)
    ...
    for (const user of users) {
      if (user.email) {
        await sendEmail(
          user.email,
          "Yangi ish e’loni!",
          `Yangi ish joyi qo‘shildi: ${work_type} (${location})\nIsh haqi: ${salary}\nBog‘lanish: ${contact}`
        );
      }
    }
