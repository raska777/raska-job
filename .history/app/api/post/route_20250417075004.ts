...
//       await sendEmail(
//         user.email,
//         'Yangi ish e’loni mavjud',
//         'Siz uchun yangi ish e’loni joylashtirildi!'
//       );
//     }
...
export async function POST(req: NextRequest) {
  try {
    ...
    // E'lonni yuborgan foydalanuvchi haqida ma'lumot
    console.log(`Ish e’loni yaratildi: ${newJob.work_type} - ${newJob.location}`);

    // Email yuborish uchun alohida route ga so'rov yuborish
    const emailResponse = await fetch('/api/email/route', {
      method: 'POST',
      body: JSON.stringify(newJob),  // E'lon ma'lumotlarini emailga yuborish
      headers: {
