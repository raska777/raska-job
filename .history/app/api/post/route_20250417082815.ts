...
//----ajoyib------
// import { getServerSession } from "next-auth";
// import { authOptions } from "@/lib/auth";  // authOptions ni import qilamiz
// import { NextRequest, NextResponse } from "next/server";
// import clientPromise from "@/lib/mongodb";
...
// POST metod (yangi ish e'lonini qo'shish va email yuborish)
export async function POST(req: NextRequest) {
  try {
    ...
    // Obuna bo'lgan foydalanuvchilarga email yuborish
    const users = await db.collection("users").find({ isSubscribed: true }).toArray();
    for (const user of users) {
      await sendEmail(
        user.email,
...
// GET metod (ish e'lonlarini olish va filtrlarni qo'llash)
export async function GET(req: NextRequest) {
  try {
