...
import styles from 'styles/joblist.module.css';
import { useSession } from 'next-auth/react';
import { FiShare2, FiPhone, FiHeart, FiLogIn } from 'react-icons/fi';
import { toast } from 'react-toastify';
import { motion } from 'framer-motion';

// ... (oldingi interfeyslar o'zgarishsiz)

const JobList = ({ selectedCity, searchQuery, toggleExpandedJob, expandedJob }: JobListProps) => {
  // ... (oldingi state va effektlar o'zgarishsiz)
  ...
  const handleCallClick = (contact: string) => {
    if (!session) {
      toast.info('Iltimos, avval tizimga kiring');
      return;
    }
  ...
  // ... (filterJobs va pagination logikasi o'zgarishsiz)

  return (
    <div className={styles.jobListContainer}>
      {/* ... (results count va boshqa elementlar o'zgarishsiz) */}
      ...
      <div className={styles.jobGrid}>
        {currentJobs.length > 0 ? (
          currentJobs.map((job, index) => (
            ...
            >
              <div className={styles.jobCardContent}>
                {/* ... (job header va asosiy ma'lumotlar o'zgarishsiz) */}
              ...
              <div className={styles.actionButtons}>
                ...
                {expandedJob === job.id && (
                  <motion.div
                    ...
                    className={styles.bottomActionButtons}
                  >
                    <button
                    ...
                    {session ? (
                      ...
                      <button
                        onClick={() => toast.info('Iltimos, avval tizimga kiring')}
                        className={`${styles.actionButton} ${styles.callButton}`}
                      >
                        <FiLogIn className={styles.actionIcon} />
                        <span>Kirish</span>
                      </button>
                    )}
          ...
          // ... (no results elementi o'zgarishsiz)
        )}
      </div>
