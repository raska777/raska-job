...
import { FiShare2, FiPhone, FiLogIn } from 'react-icons/fi';
...
const JobList = ({ selectedCity, searchQuery, toggleExpandedJob, expandedJob }: JobListProps) => {
  ...
  <a
    ...
    title={job.contact}  // Hover qilganda telefon raqamni ko'rsatish
  >
    <FiPhone size={20} />
  </a>
) : (
  <button
    ...
    className={`${styles.actionButton} ${styles.callButton} ${styles.loginRequired}`}
    aria-label="Qo'ng'iroq qilish uchun tizimga kiring"
    title="Qo'ng'iroq qilish uchun tizimga kiring"
  >
    <FiPhone size={20} />
    {/* Yashirin matn - accessibility uchun */}
    <span className="visually-hidden">Qo'ng'iroq qilish (tizimga kirish talab etiladi)</span>
  </button>
)}
                      {job.posted_date && (
                        <li className={styles.postedDate}>
