...
//     >
//         <FiPhone size={20} /> {/* Faqat icon, matnsiz */}
//     </a>
// ) : (
//     <button
//         onClick={(e) => {
...
// <FiShare2 className={`${styles.icon} ${styles.shareIcon}`} />
// Share
// </button>
// {/* Ulashish tugmasi */}
...
import styles from 'styles/joblist.module.css';
import { useSession } from 'next-auth/react';
import { FiShare2, FiPhone, FiSave, FiMessageSquare } from 'react-icons/fi';
import { FaTelegram, FaKakaoTalk } from 'react-icons/fa';
import { toast } from 'react-toastify';
import Link from 'next/link';
import 'react-toastify/dist/ReactToastify.css';
import 'styles/taostify-custom.css'; // custom CSS

// ... (keep your existing interfaces and JobListProps)

const JobList = ({ selectedCity, searchQuery, toggleExpandedJob, expandedJob }: JobListProps) => {
  // ... (keep all your existing state and useEffect code)
  ...
  const handleTelegramShare = (job: Job) => {
  ...
  const handleKakaoShare = (job: Job) => {
    // Note: Kakao sharing requires their SDK to be loaded
    toast.info('Kakao Talk ilovasi orqali ulashing', {
      icon: <FaKakaoTalk size={24} color="#3A1D1D" />,
    });
  };

  // ... (keep your existing filtering and pagination code)

  return (
    <div className={styles.jobListContainer}>
      {/* ... (keep your existing results count and grid code) */}
      ...
      {currentJobs.length > 0 ? (
        currentJobs.map((job, index) => (
          ...
          >
            <div className={styles.jobCardContent}>
              {/* ... (keep your existing job header and details code) */}
              ...
              {expandedJob === job.id && (
                <div className={styles.expandedDetails}>
                  ...
                  <div className={styles.actionButtons}>
                    ...
                    {/* Share Button - now styled like call button */}
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        handleShareJob(job);
                    ...
                    {/* Kakao Share - only show if in Korea */}
                    {navigator.language === 'ko-KR' && (
                      <button
                        ...
                        className={`${styles.actionButton} ${styles.kakaoButton}`}
                        title="KakaoTalkda ulashish"
                      >
                        <FaKakaoTalk size={18} />
                      </button>
                    )}
            ...
            {/* ... (keep your existing toggle button code) */}
          </div>
        ))
      ) : (
        {/* ... (keep your no results code) */}
      )}

      {/* ... (keep your pagination code) */}
    </div>
  );
};
