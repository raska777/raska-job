...
const handleSaveJob = async (jobId: string) => {
  try {
    if (!session) {
      toast.info(
        <div>
          <p>Iltimos, saqlash uchun tizimga kiring.</p>
          <Link
            href="/login"
    ...
    const response = await fetch('/api/saveJob', {
      ...
      body: JSON.stringify({
        jobId,
        userId: session.user.id, // yoki session.user.email - bu sizning tizimingizga bog'liq
      }),
    });

    if (!response.ok) {
      throw new Error('Ishni saqlashda xatolik yuz berdi');
    }

    const data = await response.json();
    toast.success(data.message || 'Ish muvaffaqiyatli saqlandi!');
    
    // Agar kerak bo'lsa, saqlangan ishlar ro'yxatini yangilash
    // setSavedJobs(prev => [...prev, jobId]);
    
  } catch (error) {
    console.error('Ishni saqlashda xatolik:', error);
    toast.error('Ishni saqlashda xatolik yuz berdi. Iltimos, keyinroq urunib ko\'ring.');
  }
};
  ...
  {/* Share button - same style as call button */}
  <button
    onClick={(e) => {
      e.stopPropagation();
      handleShareJob(job);
