...
const JobList = ({ selectedCity, searchQuery, toggleExpandedJob, expandedJob, lang }: JobListProps) => {
  const { data: session } = useSession();
  const [jobs, setJobs] = useState<Job[]>([]);
  const [currentPage, setCurrentPage] = useState(1);
  const [isLoading, setIsLoading] = useState(true);
    ...
    const fetchJobs = async () => {
      try {
        ...
        const translatedJobs = await Promise.all(
          validatedJobs.map((job) => getTranslatedJob(job, lang))
        );
  ...
  // UI rendering pastda qolgan
  return (
    <div className={styles.jobListContainer}> {/* qolgan qism o'zgarmagan */} </div>
  );
};
