    ...
    <FiShare2 size={20} />
         ...
         {job.posted_date && (
                        <li className={styles.postedDate}>
                          Elon qilingan: {new Date(job.posted_date).toLocaleDateString()}
                        </li>
                      )}
                    </ul>
                  </div>
...
import { useLanguage } from '@/app/components/LanguageContext'; // til konteksti
...
const JobList = ({ selectedCity, searchQuery, toggleExpandedJob, expandedJob }: JobListProps) => {
  const { data: session } = useSession();
  const { lang } = useLanguage();              // joriy til
  const [jobs, setJobs] = useState<Job[]>([]);
  const [currentPage, setCurrentPage] = useState(1);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  ...
  const translateText = async (text: string) => {
    try {
      ...
      return text; // xatoda originalni qaytar
    }
  };
  ...
  useEffect(() => {
    const fetchAndTranslate = async () => {
      ...
      try {
        ...
        const validated = data.map(job => ({
          ...
          jobname: job.jobname || 'Nomaʼlum',
          location: job.location || 'Nomaʼlum',
          work_type: job.work_type || 'Nomaʼlum',
          work_hours: job.work_hours || 'Nomaʼlum',
          salary: job.salary || 'Kelishilgan',
          language: job.language || 'Nomaʼlum',
          visa_type: job.visa_type || 'Nomaʼlum',
          contact: job.contact || 'Nomaʼlum',
          work_days: job.work_days,
          posted_date: job.posted_date
        }));

        // 3) Tarjima (agar til DEFAULT bo‘lmasa, xuddi shunday qayta o‘qishga ham to‘g‘ri keladi)
        const translatedJobs = await Promise.all(validated.map(async job => ({
          ...job,
          jobname: await translateText(job.jobname),
          location: await translateText(job.location),
          work_type: await translateText(job.work_type),
          work_hours: await translateText(job.work_hours),
          salary: await translateText(job.salary),
          language: await translateText(job.language),
          visa_type: await translateText(job.visa_type),
          work_days: job.work_days ? await translateText(job.work_days) : undefined,
        })));

        setJobs(translatedJobs);
      } catch (err: unknown) {
        console.error(err);
        setError('Ish eʼlonlarini yuklab bo‘lmadi.');
      } finally {
        setIsLoading(false);
      }
    };
    ...
    fetchAndTranslate();
  }, [lang]); // til o‘zgarganda qayta yuklanadi

  // … (qolgan kod: filtr, pagination, rendering)

  if (isLoading) return <div className={styles.loadingContainer}>…</div>;
  if (error)     return <div className={styles.errorContainer}>…</div>;

  // … keyingi rendering bloki o‘zgarmaydi
...
// const JobList = ({ selectedCity, searchQuery, toggleExpandedJob, expandedJob, selectedLang }: JobListProps) => {
//   const { data: session } = useSession();
//   const [jobs, setJobs] = useState<Job[]>([]);
//   const [currentPage, setCurrentPage] = useState(1);
//   const [isLoading, setIsLoading] = useState(true);
...
//         const data: Job[] = await res.json();
//         const translatedJobs = await Promise.all(data.map(async (job) => {
//           const translatedJob = {
//             ...job,
//             jobname: await translateText(job.jobname, selectedLang),
//             location: await translateText(job.location, selectedLang),
//             work_type: await translateText(job.work_type, selectedLang),
//             work_hours: await translateText(job.work_hours, selectedLang),
//             salary: await translateText(job.salary, selectedLang),
//             language: await translateText(job.language, selectedLang),
//             visa_type: await translateText(job.visa_type, selectedLang),
//             contact: await translateText(job.contact, selectedLang),
//             work_days: job.work_days ? await translateText(job.work_days, selectedLang) : undefined,
//           };
...
// //       return text; // if failed return the original text
// //     }
// //   };
