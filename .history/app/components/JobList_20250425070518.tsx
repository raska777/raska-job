...
//handle share
  ...
  const formatPostedDate = (timestamp: number) => {
    ...
    const hoursDiff = timeDiff / (1000 * 3600); // Soatlar farqi
    const daysDiff = Math.floor(hoursDiff / 24); // Kunlar farqi
    const minutesDiff = timeDiff / (1000 * 60); // Daqiqalar farqi
  
    if (daysDiff < 1) {
      // Agar bir kun ichida bo'lsa
      if (hoursDiff < 1) {
        return `${Math.floor(minutesDiff)} daqiqa oldin`;
      } else {
        return `${Math.floor(hoursDiff)} soat oldin`;
      }
    } else if (daysDiff === 1) {
      return "1 kun oldin";
    } else {
      return postedDate.toLocaleDateString('uz-UZ', {
        weekday: 'long',
        year: 'numeric',
        month: 'long',
        day: 'numeric'
  ...
  {job.createdAt && (
    <li className={styles.postedDate}>
      Elon qilingan: {formatPostedDate(job.createdAt.$numberLong)}
    </li>
  )}
  ...
  {job.createdAt && (
    <li className={styles.postedDate}>
      Elon qilingan: {formatPostedDate(job.createdAt)}
    </li>
  )}
