...
const JobList = ({ selectedCity, searchQuery, toggleExpandedJob, expandedJob }: JobListProps) => {
  ...
  // Yangilangan vaqtni formatlash funksiyasi
  const formatTimeAgo = (dateString?: string) => {
    ...
    try {
      ...
      console.error("Vaqtni formatlashda xato:", e);
      return "";
    }
  };
    ...
    const fetchJobs = async () => {
      try {
        ...
        const validatedJobs = data.map((job) => ({
          ...
          contact: job.contact || 'Noma\'lum',
          work_days: job.work_days,
          posted_date: job.posted_date || job.createdAt // Ikkala maydondan birini ishlatamiz
        }));
    ...
    try {
      ...
      if (navigator.share) {
        await navigator.share({
          title: `${job.work_type} 채용 공고`,
          text: shareText,
          url: jobUrl,
        });
      } else {
        await navigator.clipboard.writeText(`${shareText}\n\n${jobUrl}`);
        toast.success('채용 공고 링크가 복사되었습니다!');
      }
    } catch (err) {
          ...
          currentJobs.map((job, index) => (
            ...
            >
              ...
              <button
                ...
                className={styles.toggleButton}
              >
                  {expandedJob === job.id ? '닫기' : '자세히 보기'}              </button>
            </div>
          ))
        ) : (
          <div className={styles.noResults}>
            <div className={styles.noResultsIcon}>🔍</div>
            <p>조건에 맞는 직업이 없습니다.</p>          </div>
        )}
      </div>
