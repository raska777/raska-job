...
export default function NotificationBell() {
  ...
  const toggleSubscription = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Agar obuna bo'lsa, tasdiqlash xabarini yuborish
        if (!isSubscribed) {
          await fetch('/api/notify/subscription', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              ...
              text: 'Siz yangi ish e\'lonlaridan xabardor bo\'lish uchun obuna bo\'ldingiz!'
            }),
          });
        }
      }
  ...
  return (
    <div className="relative">
      ...
      >
        {loading ? 'Yuklanmoqda...' : isSubscribed ? '🔔 Obuna aktiv' : '🔕 Obuna bo\'lish'}
      </button>
    </div>
  );
}