...
export default function PostJob() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!phoneRegex.test(cleanedForm.contact)) {
      setError(
        "올바른 한국 전화번호를 입력해주세요! (01012345678 또는 010-1234-5678)"
      );
      return;
    }
    ...
    if (response.ok) {
      alert("작업이 MongoDB에 성공적으로 저장되었습니다! ✅");
      setForm({
        work_type: "",
        work_days: "",
        work_hours: "",
      ...
      alert(`오류 발생! 저장되지 않았습니다! ❌ ${responseData.error}`);
    }
  };
  ...
  return (
    <main className="post-job-container">
      <h2 className="form-title">직업 게시하기</h2>
