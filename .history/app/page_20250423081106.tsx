  ...
  useEffect(() => {
    ...
    const loadTranslations = async () => {
      ...
      const translatedEntries = await Promise.all(
        Object.entries(keys).map(async ([key, val]) => {
          const translated = await translateText(val, userLang);
        ...
        <div className="raskajob-header-content relative z-10">
          <div className="raskajob-nav flex justify-end p-4">
            {session ? (
              <div className="flex items-center gap-4">
                <Link href="/profile" className="raskajob-profile-icon text-white">👤</Link>
                <p className="text-white">{translations.profile}</p>
                <NotificationBell />
                <LanguageSwitcher />
              </div>
            ) : (
          ...
          <div className="raskajob-search-container p-4">
            <div className="raskajob-search-box flex flex-col md:flex-row gap-2 max-w-4xl mx-auto">
              ...
              <input
                type="text"
                placeholder={translations.ishNomi}
                className="raskajob-search-input flex-grow p-2 rounded"
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
        ...
        <div className="raskajob-content max-w-6xl mx-auto">
          <div className="flex justify-center mb-6">
            {session ? (
              ...
              <div className="text-center">
                <p className="text-gray-600 mb-2">{translations.loginToPost}</p>
                <AuthButtons />
              </div>
            )}
          </div>
          ...
          <div className="raskajob-results-header mb-4">
            {searchTerm || selectedCity ? (
              <h2 className="raskajob-results-title text-xl font-semibold">
                🔍 &quot;{searchTerm}&quot; {translations.searchResults} {selectedCity ? `${selectedCity} ${translations.in}` : translations.in}
              </h2>
            ) : (
              <h2 className="raskajob-results-title text-xl font-semibold">
