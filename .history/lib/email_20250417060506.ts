...
// lib/email.ts
export const sendJobNotification = async (email: string, work_type: string, location: string) => {
    const subject = `Yangi ish e'loni: ${work_Type}`;
    const text = `Hurmatli foydalanuvchi,\n\n"${work_type}" (${location}) nomli yangi ish e'loni platformaga qo'shildi.\n\nBatafsil: ${process.env.NEXTAUTH_URL}/jobs`;
