...
if (!process.env.EMAIL_USER || !process.env.EMAIL_PASS) {
  console.error('⚠️ EMAIL_USER va EMAIL_PASS muhit o\'zgaruvchilari topilmadi!');
  throw new Error('Email konfiguratsiyasi noto\'g\'ri');
}
...
// Transportni tekshirish
transporter.verify((error) => {
  if (error) {
    console.error('✖️ SMTP ulanishi muvaffaqiyatsiz:', error);
  } else {
    console.log('✔️ SMTP serveriga muvaffaqiyatli ulanildi');
  }
});

export const sendEmail = async (to: string, subject: string, text: string): Promise<boolean> => {
  try {
    console.log(`✉️ Email yuborilmoqda: ${to}`);
    ...
    console.log('✅ Email muvaffaqiyatli yuborildi! Message ID:', info.messageId);
    return true;
  } catch (err: unknown) {
    console.error('❌ Email yuborishda xato:', err);
