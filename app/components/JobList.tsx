...
// // JobList komponenti ichida quyidagicha qidiruvni amalga oshirishingiz mumkin:
// const filteredJobs = jobs.filter(job => {
//   const matchesCity = !selectedCity || job.location.includes(selectedCity);
//   const matchesSearch = !searchQuery || 
//     job.work_name.toLowerCase().includes(searchQuery.toLowerCase()) ||
//     job.work_type.toLowerCase().includes(searchQuery.toLowerCase()) ||
...
interface JobListProps {
  selectedCity: string;
  searchQuery: string;
  searchCategory?: string; // Yangi qo'shilgan prop
  toggleExpandedJob: (jobId: string) => void;
  expandedJob: string | null;
}
...
const JobList = ({
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      try {
        const url = `/api/post?page=${currentPage}&limit=${jobsPerPage}${selectedCity ? `&location=${encodeURIComponent(selectedCity)}` : ''
          }${searchQuery ? `&search=${encodeURIComponent(searchQuery)}` : ''
        ...
        const validatedJobs = data.jobs.map((job: any) => ({
          _id: job._id,
          work_name: job.work_name || 'Noma\'lum',
          work_type: job.work_type || 'Noma\'lum',
          location: job.location || 'Noma\'lum',
          work_hours: job.work_hours || 'Kelishilgan',
        ...
        console.error("Ish o'rinlari yuklanmadi:", err);
        setError('Ish o\'rinlari yuklanmadi. Iltimos, keyinroq urunib ko\'ring.');
      } finally {
        setIsLoading(false);
      }
    };
    ...
    <div className={styles.jobListContainer}>
      ...
      <div className={styles.jobGrid}>
        {jobs.length > 0 ? (
          ...
          <div className={styles.noJobsFound}>
            <p>Mos keladigan ish o'rinlari topilmadi.</p>
            <button
              onClick={() => {
                setCurrentPage(1);
                setError(null);
